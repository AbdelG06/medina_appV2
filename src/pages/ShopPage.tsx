    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 pt-28 pb-16">
        <div className="mb-8 rounded-3xl border border-border bg-gradient-to-r from-card to-muted/40 p-6 md:p-8">
          <p className="text-xs uppercase tracking-[0.22em] text-moroccan-ochre-dark font-semibold mb-2">{t("E-shop", "E-shop")}</p>
          <h1 className="font-heading text-3xl md:text-5xl font-bold mb-3">{t("Boutique de la Medina", "Medina Shop")}</h1>
          <p className="text-sm md:text-base text-muted-foreground max-w-3xl">
            {t(
              "Produits artisanaux verifies avec prix clairs, artisans identifies et contact direct.",
              "Verified handcrafted products with clear prices, identified artisans and direct contact.",
            )}
          </p>
            ...
            return (
              <section key={category} className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">{items.length} {t("articles", "items")}</span>
                </div>
                    ...
                    <article key={item.id} className="group rounded-2xl border border-border bg-card overflow-hidden shadow-sm hover:shadow-moroccan transition-all">
                      ...
                      <div className="p-5 space-y-3">
                        ...
                        <div className="rounded-lg border border-border bg-background px-3 py-2 text-sm">
                          <p><span className="font-semibold">{t("Artisan", "Artisan")}: </span>{getText(item.artisanName)}</p>
                          <p className="text-muted-foreground"><span className="font-semibold text-foreground">{t("Adresse", "Address")}: </span>{getText(item.artisanAddress)}</p>
                        </div>
                        {item.phone ? (
                          <a
