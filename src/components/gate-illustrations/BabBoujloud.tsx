const BabBoujloud = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 260 380" className={className} xmlns="http://www.w3.org/2000/svg">
    {/* Background wall */}
    <rect x="0" y="0" width="260" height="380" fill="#D4C4A0" rx="2" />
    
    {/* Crenellations on top */}
    {[0, 26, 52, 78, 104, 130, 156, 182, 208, 234].map((x) => (
      <rect key={x} x={x} y="0" width="20" height="25" fill="#C4B48E" stroke="#B8A67A" strokeWidth="0.5" />
    ))}
    <rect x="0" y="25" width="260" height="8" fill="#C4B48E" />

    {/* Main zellige panel - blue */}
    <rect x="15" y="40" width="230" height="240" rx="3" fill="#1A4B8C" />
    {/* Zellige geometric pattern */}
    {Array.from({ length: 12 }).map((_, row) =>
      Array.from({ length: 11 }).map((_, col) => (
        <g key={`${row}-${col}`} transform={`translate(${20 + col * 20}, ${45 + row * 19})`}>
          <rect x="1" y="1" width="16" height="16" fill="none" stroke="#3B7DD8" strokeWidth="0.5" />
          <polygon points="8,2 14,8 8,14 2,8" fill="none" stroke="#5BA0E8" strokeWidth="0.4" />
          <circle cx="8" cy="8" r="2" fill="none" stroke="#7BB8F0" strokeWidth="0.3" />
        </g>
      ))
    )}

    {/* Green decorative band */}
    <rect x="15" y="145" width="230" height="18" fill="#1B6B3A" opacity="0.8" />
    {Array.from({ length: 23 }).map((_, i) => (
      <polygon key={i} points={`${20 + i * 10},148 ${25 + i * 10},155 ${20 + i * 10},162`} fill="#2D9B55" opacity="0.6" />
    ))}
    ...
    {/* Minaret visible through arch */}
    <rect x="122" y="120" width="16" height="60" fill="#3A7A4A" opacity="0.4" />
    <rect x="119" y="115" width="22" height="8" fill="#3A7A4A" opacity="0.3" />
  </svg>
);

export default BabBoujloud;
