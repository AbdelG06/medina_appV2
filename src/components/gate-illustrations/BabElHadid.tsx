const BabElHadid = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 260 380" className={className} xmlns="http://www.w3.org/2000/svg">
    {/* Background wall */}
    <rect x="0" y="0" width="260" height="380" fill="#CCC4B0" rx="2" />

    {/* Crenellations on top */}
    {[0, 26, 52, 78, 104, 130, 156, 182, 208, 234].map((x) => (
      <rect key={x} x={x} y="0" width="20" height="25" fill="#BCB4A0" stroke="#B0A894" strokeWidth="0.5" />
    ))}
    <rect x="0" y="25" width="260" height="8" fill="#BCB4A0" />

    {/* Main zellige panel - burgundy */}
    <rect x="15" y="40" width="230" height="240" rx="3" fill="#6B2C3C" />
    {/* Zellige geometric pattern */}
    {Array.from({ length: 12 }).map((_, row) =>
      Array.from({ length: 11 }).map((_, col) => (
        <g key={`${row}-${col}`} transform={`translate(${20 + col * 20}, ${45 + row * 19})`}>
          <rect x="1" y="1" width="16" height="16" fill="none" stroke="#8B4C5C" strokeWidth="0.5" />
          <polygon points="8,2 14,8 8,14 2,8" fill="none" stroke="#AB6C7C" strokeWidth="0.4" />
          <circle cx="8" cy="8" r="2" fill="none" stroke="#CB8C9C" strokeWidth="0.3" />
        </g>
      ))
    )}

    {/* Iron decorative band - dark metallic */}
    <rect x="15" y="145" width="230" height="18" fill="#3C3C3C" opacity="0.9" />
    {Array.from({ length: 23 }).map((_, i) => (
      <polygon key={i} points={`${20 + i * 10},148 ${25 + i * 10},155 ${20 + i * 10},162`} fill="#5C5C5C" opacity="0.7" />
    ))}
