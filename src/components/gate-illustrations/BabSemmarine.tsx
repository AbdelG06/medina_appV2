const BabSemmarine = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 260 380" className={className} xmlns="http://www.w3.org/2000/svg">
    {/* Background wall */}
    <rect x="0" y="0" width="260" height="380" fill="#D8C8A8" rx="2" />

    {/* Crenellations on top */}
    {[0, 26, 52, 78, 104, 130, 156, 182, 208, 234].map((x) => (
      <rect key={x} x={x} y="0" width="20" height="25" fill="#C8B898" stroke="#BCA888" strokeWidth="0.5" />
    ))}
    <rect x="0" y="25" width="260" height="8" fill="#C8B898" />

    {/* Main zellige panel - terracotta */}
    <rect x="15" y="40" width="230" height="240" rx="3" fill="#8B5A3C" />
    {/* Zellige geometric pattern */}
    {Array.from({ length: 12 }).map((_, row) =>
      Array.from({ length: 11 }).map((_, col) => (
        <g key={`${row}-${col}`} transform={`translate(${20 + col * 20}, ${45 + row * 19})`}>
          <rect x="1" y="1" width="16" height="16" fill="none" stroke="#A87A5C" strokeWidth="0.5" />
          <polygon points="8,2 14,8 8,14 2,8" fill="none" stroke="#C89A7C" strokeWidth="0.4" />
          <circle cx="8" cy="8" r="2" fill="none" stroke="#D8AA8C" strokeWidth="0.3" />
        </g>
      ))
    )}

    {/* Brown decorative band */}
    <rect x="15" y="145" width="230" height="18" fill="#654321" opacity="0.8" />
    {Array.from({ length: 23 }).map((_, i) => (
      <polygon key={i} points={`${20 + i * 10},148 ${25 + i * 10},155 ${20 + i * 10},162`} fill="#8B6914" opacity="0.6" />
    ))}
