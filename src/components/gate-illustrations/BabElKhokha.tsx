const BabElKhokha = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 260 380" className={className} xmlns="http://www.w3.org/2000/svg">
    {/* Background wall */}
    <rect x="0" y="0" width="260" height="380" fill="#D2C6A4" rx="2" />

    {/* Crenellations on top */}
    {[0, 26, 52, 78, 104, 130, 156, 182, 208, 234].map((x) => (
      <rect key={x} x={x} y="0" width="20" height="25" fill="#C2B694" stroke="#B6AA88" strokeWidth="0.5" />
    ))}
    <rect x="0" y="25" width="260" height="8" fill="#C2B694" />

    {/* Main zellige panel - olive green */}
    <rect x="15" y="40" width="230" height="240" rx="3" fill="#556B2F" />
    {/* Zellige geometric pattern */}
    {Array.from({ length: 12 }).map((_, row) =>
      Array.from({ length: 11 }).map((_, col) => (
        <g key={`${row}-${col}`} transform={`translate(${20 + col * 20}, ${45 + row * 19})`}>
          <rect x="1" y="1" width="16" height="16" fill="none" stroke="#758B4F" strokeWidth="0.5" />
          <polygon points="8,2 14,8 8,14 2,8" fill="none" stroke="#95AB6F" strokeWidth="0.4" />
          <circle cx="8" cy="8" r="2" fill="none" stroke="#B5CB8F" strokeWidth="0.3" />
        </g>
      ))
    )}

    {/* Ochre decorative band */}
    <rect x="15" y="145" width="230" height="18" fill="#CC7722" opacity="0.8" />
    {Array.from({ length: 23 }).map((_, i) => (
      <polygon key={i} points={`${20 + i * 10},148 ${25 + i * 10},155 ${20 + i * 10},162`} fill="#DD9955" opacity="0.6" />
    ))}
